...
namespace BallTracking.Runtime
{
    public static class QuestVideoEncoderProbe
    {
        ...
        public sealed class CodecCapability
        {
            ...
            public string preferred_encoder_name;
            public bool preferred_encoder_hardware_accelerated;
            public bool preferred_encoder_software_only;
            public bool preferred_encoder_vendor;
            public string[] encoder_names;
            public string note;
        }
        ...
        public static ProbeResult Probe()
        {
            ...
            {
                android_api_level = GetAndroidApiLevel(),
        ...
        public static string Summarize(ProbeResult result)
        {
            ...
            var avcNames = result.avc?.encoder_names?.Length ?? 0;
            var hevcNames = result.hevc?.encoder_names?.Length ?? 0;
            return
                $"api {result.android_api_level} | " +
                $"avc_supported {(result.avc?.supported == true ? 1 : 0)} | " +
                $"avc_preferred {FormatValue(result.avc?.preferred_encoder_name)} | " +
                $"avc_hw {(result.avc?.preferred_encoder_hardware_accelerated == true ? 1 : 0)} | " +
                $"avc_encoders {avcNames} | " +
                $"hevc_supported {(result.hevc?.supported == true ? 1 : 0)} | " +
                $"hevc_preferred {FormatValue(result.hevc?.preferred_encoder_name)} | " +
                $"hevc_hw {(result.hevc?.preferred_encoder_hardware_accelerated == true ? 1 : 0)} | " +
        ...
        private static CodecCapability ProbeCodec(string mime)
        {
            var capability = new CodecCapability
            {
                mime = mime,
                supported = false,
                preferred_encoder_name = string.Empty,
                preferred_encoder_hardware_accelerated = false,
                preferred_encoder_software_only = false,
                preferred_encoder_vendor = false,
                encoder_names = Array.Empty<string>(),
                note = string.Empty,
            };
                ...
                foreach (var codecInfo in codecInfos)
                {
                    using (codecInfo)
                    {
                        ...
                        foreach (var supportedType in supportedTypes)
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(capability.preferred_encoder_name))
                            {
                                capability.preferred_encoder_name = encoderName;
                                capability.preferred_encoder_hardware_accelerated = TryGetBool(codecInfo, "isHardwareAccelerated");
                                capability.preferred_encoder_software_only = TryGetBool(codecInfo, "isSoftwareOnly");
                                capability.preferred_encoder_vendor = TryGetBool(codecInfo, "isVendor");
                            }
                        }
                    }
                }

                capability.encoder_names = encoderNames.ToArray();
