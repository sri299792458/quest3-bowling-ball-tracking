...
namespace
{
    ...
    int64_t g_presentationTimeNs = 0;
    PFNEGLPRESENTATIONTIMEANDROIDPROC g_eglPresentationTimeAndroid = nullptr;
    ...
    void RenderIntoEncoderSurface()
    {
        ...
        {
            g_eglPresentationTimeAndroid = reinterpret_cast<PFNEGLPRESENTATIONTIMEANDROIDPROC>(
        ...
        if (g_presentationTimeNs >= 0 && g_eglPresentationTimeAndroid != nullptr)
        {
            g_eglPresentationTimeAndroid(display, g_encoderSurface, g_presentationTimeNs);
        }
...
extern "C"
{
    ...
    bool SQB_SetEncoderSurface(void* surfaceObject)
    {
        ...
        if (g_javaVm->AttachCurrentThread(&env, nullptr) != JNI_OK || env == nullptr)
        {
    ...
    void SQB_SetPresentationTimeUs(int64_t presentationTimeUs)
    {
        std::lock_guard<std::mutex> lock(g_gate);
        g_presentationTimeNs = presentationTimeUs * 1000LL;
    }
