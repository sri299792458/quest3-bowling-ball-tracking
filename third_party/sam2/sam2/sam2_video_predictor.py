...
class SAM2VideoPredictorVOS(SAM2VideoPredictor):
    ...
    def _compile_all_components(self):
        ...
        skip_memory_attention_compile = os.environ.get(
            "SAM2_SKIP_MEMORY_ATTENTION_COMPILE"
        )
        if skip_memory_attention_compile is None:
            skip_memory_attention_compile = os.name == "nt"
        else:
            skip_memory_attention_compile = skip_memory_attention_compile == "1"

        if skip_memory_attention_compile:
            print(
                "Skipping torch.compile for memory_attention on this setup to avoid "
                "dynamic-shape recompilation instability."
            )
        else:
