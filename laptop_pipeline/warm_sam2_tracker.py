...
def _list_jpg_frames(source_path: str):
...
def get_source_metadata(source_path: str, preview_fps: Optional[float] = None):
    ...
    if source.is_dir():
        ...
        return len(frame_files), video_width, video_height, fps if fps > 0 else 30.0
    ...
    capture.release()
    return frame_count, video_width, video_height, fps if fps > 0 else float(preview_fps or 30.0)
...
class WarmSam2Tracker:
    def __init__(self, config: Optional[WarmSam2Config] = None):
        self.config = config or WarmSam2Config()
        self._torch = None
    ...
    def track_from_seed(self, video_path: str, seed: dict, output_dir: Path, no_preview: bool = True, frame_limit: int = 0, preview_fps: Optional[float] = None):
        self._ensure_loaded()
        ...
        with summary_path.open("w", encoding="utf-8") as handle:
            handle.write(f"mode=batch_video_predictor\n")
